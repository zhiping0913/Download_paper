...
# -*- coding: utf-8 -*-
"""
从Nature HTML中提取并转换Data availability
"""
...
def extract_data_availability_from_html(html_file: str) -> str:
    """从HTML中提取Data availability并转换为Markdown"""
    ...
    # 找到Data availability部分
    data_avail_section = soup.find('section', {'data-title': 'Data availability'})
    if not data_avail_section:
        print("❌ 未找到Data availability部分")
        return ""

    # 找到内容div
    content_div = data_avail_section.find('div', {'class': 'c-article-section__content'})
    if not content_div:
        print("❌ 未找到Data availability内容")
        return ""

    # 获取所有段落
    paragraphs = content_div.find_all('p')
    if not paragraphs:
        print("❌ 未找到Data availability段落")
        return ""
    ...
    print(f"✓ Data availability转换完成: {len(result)} 字符")
...
def main():
    ...
    print("=" * 80)
    print("📖 提取并转换Data availability")
    print("=" * 80)

    print("\n1️⃣  从HTML中提取Data availability...")
    data_avail_md = extract_data_availability_from_html(html_file)
