...
7. **提取参考文献** — 尊重原网页的输出。**每篇文章只在 Step 0 访问一次 Crossref**，不再逐条拿 DOI 去查 Crossref / Semantic Scholar；页面只给裸字符串的出版商，用 Step 0 那一次响应里的 `references` 离线生成 BibTeX
...
9. **References** — 编号引用（页面已给完整引文的只输出原文；其余附离线生成的 BibTeX 代码块）
