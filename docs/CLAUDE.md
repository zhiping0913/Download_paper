...
| `publisher/wildcard.py` | 跨 publisher 共享函数：正文查找、摘要提取、公式转换、引文文本生成 |
| `publisher/__init__.py` | 模块导出 |
| `core/utilities.py` | Crossref / Semantic Scholar 查询、输出目录组织、`metadata.json` 与 `crossref.json` 保存 |
...
7. **提取参考文献** — 尊重原网页的写法并保留链接，**不生成 BibTeX 代码块**。每篇文章只在 Step 0 访问一次 Crossref，不逐条核验；页面没有引文文本时，用那一次响应离线生成可读文本
...
9. **References** — 编号引用，原文写法 + 链接（无 BibTeX 块）
