...
- ⚠️ **一篇之内的重试原本完全没有节流**。`retry_download` 的 `sleep` 只写在
  ...
  `DP_RETRY_DELAY`（默认 30 秒），不递增**
- ⚠️ 图片**共用** `DP_RETRY_DELAY`。默认 30 秒后，一张确实取不到的图要花
  `2 × 30` 秒（`DP_IMG_MAX_RETRIES=3`），而不是原先的几秒。这是给 PDF 那条路
  加节流必然付的代价；要让图片快起来，就在它的调用点单独传一个更小的值，
  别把这个默认值调小 —— 那等于把 PDF 的节流一起撤掉
