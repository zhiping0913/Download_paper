...
- ⚠️ **但段落里的公式图不下载**（判据 `_is_downloadable_asset`：在 `<figure>` 里，
  或在 `div.display-formula` 里，才下）。符号大小的 JPEG 单独存下来没有用处 ——
  要还原公式得对它 OCR。这和 SPIE 的做法是同一条：**引用出版商的 URL**。
  ⚠️ 引用时要**改写 `<img>` 的 src 成绝对地址、让 pandoc 自己渲染**，
  不能把 `![](url)` 当文本塞进 HTML 片段 —— 那对 pandoc 是字面量，会被转义成
  `\![\](https://…)`。实测代价：SC25 那篇从下 27 张图变成下 8 张（19 个行内
  `imginl*.svg` 改为远程引用）
