...
# ---------------------------------------------------------------------------
# 1. 取数阶梯（4 层回退）—— PDF / 图片 / 补充材料 / API 页面共用同一套顺序
# ---------------------------------------------------------------------------
#
...
# 两个老开关，和上面是同一套阶梯的另一种表达：
# export DP_HTTP_FIRST=0                 # = 摘掉 request 层。默认 1
# export DP_PDF_FRESH_CHROME=0           # = 摘掉 fresh 层（连带 referer 也没得用）。默认 1
# export DP_HTTP_USER_AGENT="Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/152.0.0.0 Safari/537.36"

# ---------------------------------------------------------------------------
# 2. Chrome —— 程序会用到【两个】Chrome 实例，各占一个端口
# ---------------------------------------------------------------------------
#
#   主实例      CHROME_DEBUG_PORT      打开论文页面、提取正文；每篇结束后关闭重开
#   辅助实例    CHROME_AUX_DEBUG_PORT  阶梯里的 fresh / referer 两层：PDF、图片、
#                                      补充材料、API 页面；用 aux_dir，每次重建，
...
# 不设 → 默认 /tmp/dp_profiles_xxxxxx，后缀按启动时间+pid 做种随机生成，
...
# 加上 --password-store=gnome-libsecret 后恢复到 1304 条、其中 184 条是出版商的）。
# 程序在 Linux + 有 DBUS 会话时自动加；取值因机器而异（kwallet5 在某些机器上无效），
# 置空则完全不加。
...
export DP_CLOUDFLARE_TIMEOUT=60          # 默认 60。⚠️ 它同时决定「等 widget 出现」
                                         # 的窗口（本值/7.5，此处 8s）—— 没有挑战的
                                         # 页面要等满这个窗口才能断定无挑战，而绝大多数
                                         # 页面都没有挑战，所以别随手调大（这里曾是
                                         # 600，等于每个干净页面白等 80 秒）

# PDF 快路径探测：一次性 Chrome 启动后、附着 CDP 点验证框之前，先等这么久
# 看 PDF 会不会自己落盘
export DP_PDF_FASTPATH_WAIT=5            # 默认 5。它让「下载完成」和「出现真实页面」
                                         # 赛跑：文件落盘就完全不附着 CDP；若先出现
                                         # 页面（挑战/付费墙）则立即转 CDP，不白等
...
# 补充材料：每个链接的 goto + download 事件等待 / 下载完成等待
export DP_SUPPLEMENTAL_TIMEOUT=60                # 默认 60
export DP_SUPPLEMENTAL_DOWNLOAD_COMPLETE_TIMEOUT=120  # 默认 120；大 DOCX/MP4 再调大

# 图片 CDN 的 goto / 重取
export DP_FIGURE_TIMEOUT=60              # 默认 60。它同时是图片走 request 层时的
                                         # 读超时；该层的总时限另取本值 ×3

# ---------------------------------------------------------------------------
# 4b. 两条【死锁断路器】—— 和上面那些不是一类东西
# ---------------------------------------------------------------------------
# 上面那些是「这一步该花多久」。下面这两条是「这一步永远不会结束」时的兜底，
# 补的是两个本来【没有任何上限】的洞：
#
#   page.evaluate() / response.body() / download.path()
#       —— Playwright 里仅有的三个不接受 timeout=、也不受 set_default_timeout
#          管辖的调用。网络一抖，页面内的 fetch() promise 永不 settle，
#          evaluate 就永不返回，整批任务停在那里（而不是报错重试）
#   _http_download_to() 的流式循环
#       —— requests 的读超时只在【套接字空闲】时触发，服务器一点一点吐字节就
#          永远不算空闲。实测：每 0.4 秒 1 字节，在 timeout=(15,2) 下连流 24 秒，
#          2 秒读超时一次都没响。补充材料里的视频最容易踩
#
# ⚠️ 慢链路请把它们调【大】，别调小 —— 一个 500 MB 的真视频必须还能下完。
export DP_INPAGE_FETCH_TIMEOUT=90        # 默认 90。单次页面内 fetch / 读响应体的上限
export DP_HTTP_TOTAL_TIMEOUT=600         # 默认 600。单个直接下载的总时限
...
# export CAPTURED_DATA_DIR=captured_data                    # 子目录名。默认 captured_data
...
echo "──────────────────────────────────────────────────────"
for v in DP_FETCH_ORDER DP_FETCH_PDF DP_FETCH_FIGURE \
         DP_FETCH_SUPPLEMENT DP_FETCH_API \
         DP_HTTP_FIRST DP_PDF_FRESH_CHROME DP_HTTP_USER_AGENT \
         CHROME_PATH CHROME_DEBUG_PORT CHROME_AUX_DEBUG_PORT \
         CHROME_PROFILE_ROOT CHROME_PROFILE CHROME_DOWNLOAD_DIR \
...
#           地址）、"pdf_link"（直接给 PDF 地址，连论文页面都不访问）、
#           "referer"（第 4 层点击跳转的来路页）和 "header"（合并进请求头，
#           其中的 referer 在顶层 referer 缺失时被采用）
#
# 其它开关：
#   --output PATH             覆盖输出目录
...
#
# 只跑某一层（调试用，注意截断语义）：
# DP_FETCH_PDF=referer python complete_paper_extraction.py --doi 10.1088/0741-3335/51/3/035013
# DP_FETCH_PDF=tab     python complete_paper_extraction.py --doi 10.1109/JLT.2023.3234567
