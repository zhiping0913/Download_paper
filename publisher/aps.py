...
# ⚠️ get_supplemental_links() is gone. It navigated the article page away to
# /supplemental/{doi} and scraped a[data-id] plus a regex over innerText --
# and it has had no callers since the handler started reading the listing out
# of the capture (the page fetches that endpoint for itself on every load).
# Its page.content() was the last live-DOM read left in this file.
...
class APSHandler(PublisherHandler):
    ...
    def journal_prefix_from_url(url: str) -> str:
        ...
        the supplemental link used to be built the same way, by rewriting
        /abstract/ to /supplemental/ in the URL.
        """
        if not url:
            return ''
